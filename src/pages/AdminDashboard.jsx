...
const MOCK_PARTNERS = [
  ...
  { id: 4, name: "Hana Girma", territory: "Ethiopia / Addis Ababa", stage: "Identified", lastContact: "â€”", nextFollowup: "This week", languages: ["Amharic","Oromo"] },
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {/* Follow-ups due */}
        <Card>
          <div style={{ fontSize: 18, fontWeight: 600, color: "#a0c8e8", letterSpacing: "0.08em", marginBottom: 16 }}>FOLLOW-UPS DUE</div>
          {overdue.map(p => (
            <div key={p.id} onClick={() => navigate("partner-profile", { partnerId: p.id })}
              ...
              <div>
                <div style={{ fontSize: 20, fontWeight: 600, color: "#ffffff" }}>{p.name}</div>
                <div style={{ fontSize: 18, color: "#a0c8e8" }}>{p.territory}</div>
              </div>
              <span style={{ fontSize: 17, background: "rgba(220,80,80,0.2)", color: "#f07070", padding: "3px 10px", borderRadius: 20, fontWeight: 600 }}>Overdue</span>
            </div>
          ))}
          {today.map(p => (
            <div key={p.id} onClick={() => navigate("partner-profile", { partnerId: p.id })}
              ...
              <span style={{ fontSize: 17, background: "rgba(240,180,60,0.12)", color: "#f0c040", padding: "3px 10px", borderRadius: 20, fontWeight: 600 }}>Soon</span>
            </div>
          ))}
          {overdue.length === 0 && today.length === 0 && (
            <p style={{ fontSize: 19, color: "#7ab0cc", textAlign: "center", padding: "20px 0" }}>All caught up âœ“</p>
          )}
        </Card>
        ...
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <button onClick={() => navigate("partners")} style={{ fontSize: 18, color: "#5ab0f0", background: "none", border: "none", cursor: "pointer" }}>View all â†’</button>
          </div>
          {MOCK_PARTNERS.slice(0, 5).map(p => (
            <div key={p.id} onClick={() => navigate("partner-profile", { partnerId: p.id })}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 19, fontWeight: 600, color: "#ffffff" }}>{p.name}</div>
                  <div style={{ fontSize: 17, color: "#7ab0cc" }}>{p.territory}</div>
                </div>
              </div>
